...
RUN --mount=type=cache,target=/var/cache/apk sh -ex <<'EOS'
# load module: ngx_http_internal_redirect_module.so
sed -i '/events {/i load_module "/usr/lib/nginx/modules/ngx_http_internal_redirect_module.so";' /etc/nginx/nginx.conf
EOS
