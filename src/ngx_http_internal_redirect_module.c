...
static char *ngx_http_internal_redirect_merge_conf(ngx_conf_t *cf, void *parent,
    void *child);
...
ngx_http_internal_redirect_precontent_handler(ngx_http_request_t *r)
{
    ...
    conf = ngx_http_get_module_loc_conf(r, ngx_http_internal_redirect_module);

    if (conf->location == NULL) {
        return NGX_DECLINED;
    }

    if (ngx_http_complex_value(r, conf->location, &location) != NGX_OK) {
        return NGX_DECLINED;
    }
...
ngx_http_internal_redirect_post_conf(ngx_conf_t *cf)
{
    if (nxe_phase_add_handler(cf, NGX_HTTP_PRECONTENT_PHASE,
        NXE_PHASE_PRIO_INTERNAL_REDIRECT,
        ngx_http_internal_redirect_precontent_handler, "internal_redirect")
        != NGX_OK)
    {
        return NGX_ERROR;
    }
